...
function lookup (buffer, filepath, filetype) {
 ...
 */
module.exports = function (input, filetype, callback) {
  ...
  if (typeof callback === 'function') {
    asyncFileToBuffer(filepath, buffer, function () {
      ...
      try{
        callback(null, lookup(buffer, filepath, filetype));
      } catch(err){
        callback(err,null);
      }
    });
    ...
    // return the dimensions
    try{
      return lookup(buffer, filepath, flietype);
    } catch(err){
      callback(err,null);
    }
  }
